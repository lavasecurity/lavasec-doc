...
## 1. 철학: 차분한 핵심, 노력으로 얻는 깊이 {#1-philosophy-calm-core-earned-depth}
...
### 오늘날 존재하는 토큰화된 계층 **(구현됨)** {#the-tokenized-layer-that-exists-today-implemented}
...
## 2. LavaTier — Floor / Window / Workshop **(구현됨)** {#2-lavatier-floor-window-workshop-implemented}
...
## 3. Soft Shield Guardian 마스코트 **(구현됨)** {#3-the-soft-shield-guardian-mascot}
...
### 3.1 7가지 표정 상태 {#31-the-7-expression-states}
...
### 3.2 연결성 → 표정 매핑 (6 → 4) {#32-connectivity-expression-mapping-6-4}
...
### 3.3 스킨(룩) **(구현됨)** {#33-skins-looks-implemented}
...
### 3.4 개인정보 가림 **(구현됨)** {#34-privacy-redaction-implemented}
...
### 3.5 이 트리에 없음 **(계획됨)** {#35-not-in-this-tree-planned}
...
### 4.2 DNS 전송 레이블 {#42-dns-transport-labels}
...
### 4.3 번역 금지 용어 {#43-do-not-translate-terms}
...
## 6. 국제화 **(진행 중)** {#6-internationalization-in-progress}
...
## 7. 참고 산출물 {#7-reference-artifacts}
