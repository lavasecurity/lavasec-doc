...
Lava 面向的是不太懂技术的普通用户 —— 比如家长、长辈 —— 设计也由此而来。日常那一层界面对每个人来说都「直接就能用」，安安静静；更多的细节、惊喜和控制项，只有当用户主动去找时才会显现出来（**靠探索解锁**）。不催促、不告警，技术机制在你主动寻找之前一直隐身。
...
品牌和协议术语在 **所有** 语言里都钉死保持原样。本地化词汇表的「不翻译」清单是权威，它钉死了：**Lava Security、Lava Security LLC、lavasecurity.app、support@lavasecurity.app、legal@lavasecurity.app、DNS、VPN、DoH、TCP、Apple、Google、Cloudflare、Quad9、The Block List Project、Phishing.Database、HaGeZi、OISD、AdGuard、1Hosts、StevenBlack。**
...
1. **网上有岩浆**（`lava`）—— 把危险包装成一个比喻；主操作是「认识 Lava」。
2. **Lava 在这里守护你**（`guardIntro`）—— 守护者登场的那一刻。
