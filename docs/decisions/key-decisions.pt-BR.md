...
**Justificativa.** Tratar a Lava como um motor de filtragem local / user agent — em vez de uma distribuidora de blocklist — minimiza a redistribuição sob GPLv3 e a exposição na App Review. O dispositivo busca cada lista por TLS diretamente de seu `source_url` curado e faz o parse localmente sob limites estritos de tamanho/regras; as listas da comunidade são aceitas conforme servidas (os `accepted_source_hashes` do catálogo são consultivos, não uma barreira rígida — um único hash fixado não consegue acompanhar um upstream que rotaciona rapidamente e só produzia rejeições falsas), enquanto o tier de barreira contra ameaças da Lava permanece com hash fixado. A proveniência é imposta no catálogo, não por uma barreira de hash no cliente: uma mudança de `source_url` que coloque um **publicador diferente** por trás de uma entrada existente deve usar um novo `list_id`, para que um dispositivo nunca passe a baixar em silêncio os bytes de outra parte sob uma identidade que a pessoa usuária já aceitou. Uma mudança **entre hosts dentro do próprio namespace do mesmo publicador** mantém o seu `list_id` — curador, nomes de arquivo e conteúdo seguem iguais, muda apenas a CDN à frente, e reidentificar isso retiraria cada lista afetada nos dispositivos que já a tinham ativado (os IDs retirados vão para quarentena, então a pessoa perde em silêncio uma lista que escolheu). Mudanças dentro do mesmo publicador são registradas aqui em vez de reidentificadas, com a evidência de propriedade first-party. Cada conjunto de regras analisado também passa por um filtro de domínios protegidos para que uma lista upstream não possa bloquear domínios da Lava/Apple/provedor de identidade. O modelo é imposto na CI por `check-gpl-blocklist-distribution.sh` (sem código de espelhamento, sem URLs de artefatos hospedados pela Lava, sem fontes GPL habilitadas por padrão, sem gravações de bytes no R2).

**Mudanças registradas dentro do mesmo publicador.**

| Data | Listas | De → Para | Evidência first-party |
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@37522026.221.30748` | O caminho `/gh/<owner>/<repo>` do jsDelivr resolve exatamente para aquele repositório do GitHub, então o publicador fica fixado **pela própria URL** e nenhum terceiro pode ser servido sob ela — é uma CDN à frente da fonte original, não outra origem. O GitHub bloqueou a conta `hagezi`, de modo que o host raw agora dá 404 enquanto o jsDelivr segue servindo sua cópia em cache do mesmo repositório fixada na versão **imutável** `37522026.221.30748`, cujo conteúdo é o de 2026-08-09. Não `@latest`: o jsDelivr resolve esse alias pela API do GitHub, que já retorna 404 para este repositório, então ele se sustenta apenas na resolução em cache e pode parar de resolver sem aviso, desfazendo em silêncio este reparo. Uma versão fixada é mantida em cache permanentemente e não pode rodar por baixo de nós — o conteúdo fica congelado, e voltar a uma origem viva é uma mudança de catálogo deliberada. O HostlistsRegistry da AdGuard fez a mesma substituição para todas as listas do HaGeZi em 2026-08-10. Um espelho no GitLab com namespace homônimo foi avaliado e **rejeitado**: nome de usuário coincidente, idade do repositório, metadados de commit e cabeçalhos copiados são todos reproduzíveis por um terceiro, e nada disso é autenticado por assinatura ou link de primeira parte. |
