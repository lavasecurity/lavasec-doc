...
> 對象：工程師與管理層。這是 Lava Security 背後承重設計決策的 ADR 式記錄——那些形塑了架構、隱私承諾或產品邊界，尤其是曾經嘗試又被回退的決策。每一條都列出**決策**、其**脈絡**、**理由**，以及一個取自專案狀態圖例的**狀態**（採用 / 回退 / 取代 / 提案）。
>
...
**脈絡。** iOS 一直默默地停止通道（reason 17），而約 45 分鐘內無物將其重啟，使使用者失去防護。貿然啟用隨需會讓 VPN 無法關閉，而失敗即放行的預設又會在空窗期放行流量。
