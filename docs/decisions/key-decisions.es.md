...
**Justificación.** Tratar a Lava como un motor de filtrado local / agente de usuario — en lugar de un distribuidor de listas de bloqueo — minimiza la redistribución GPLv3 y la exposición ante App Review. El dispositivo descarga cada lista por TLS directamente desde su `source_url` curada y la analiza localmente bajo límites estrictos de tamaño/reglas; las listas comunitarias se aceptan tal como se sirven (los `accepted_source_hashes` del catálogo son orientativos, no una barrera estricta — un único hash fijado no puede seguir el ritmo de un upstream que rota rápido y solo producía rechazos falsos), mientras que el nivel de barrera contra amenazas de Lava se mantiene fijado por hash. La procedencia se aplica en el catálogo, no mediante una barrera de hash del cliente: un cambio de `source_url` que ponga a un **editor distinto** detrás de una entrada existente debe usar un nuevo `list_id`, para que un dispositivo nunca empiece a descargar en silencio los bytes de otra parte bajo una identidad que la persona usuaria ya aceptó. Un traslado **entre hosts dentro del propio espacio de nombres del mismo editor** conserva su `list_id`: el curador, los nombres de archivo y el contenido no cambian, solo cambia la CDN que está delante, y volver a identificarlo retiraría cada lista afectada en los dispositivos que ya la habían activado (los IDs retirados quedan en cuarentena, así que la persona usuaria pierde en silencio una lista que eligió). Los traslados dentro del mismo editor se registran aquí en lugar de reidentificarse, junto con la evidencia de propiedad de primera parte. Cada conjunto de reglas analizado también se pasa por un filtro de dominios protegidos para que una lista upstream no pueda bloquear dominios de Lava/Apple/proveedores de identidad. El modelo se aplica en CI mediante `check-gpl-blocklist-distribution.sh` (sin código de réplica, sin URL de artefactos alojados por Lava, sin fuentes GPL habilitadas por defecto, sin escrituras de bytes en R2).

**Traslados registrados dentro del mismo editor.**

| Fecha | Listas | De → A | Evidencia de primera parte |
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@37522026.221.30748` | La ruta `/gh/<owner>/<repo>` de jsDelivr resuelve exactamente a ese repositorio de GitHub, así que el editor queda fijado **por la propia URL** y ningún tercero puede servirse bajo ella: es una CDN delante de la fuente original, no otro origen. GitHub bloqueó la cuenta `hagezi`, de modo que el host raw ahora da 404 mientras jsDelivr sigue sirviendo su copia en caché del mismo repositorio fijada a la versión **inmutable** `37522026.221.30748`, cuyo contenido es el del 2026-08-09. No `@latest`: jsDelivr resuelve ese alias a través de la API de GitHub, que ya devuelve 404 para este repositorio, así que sobrevive solo por resolución en caché y podría dejar de resolverse sin aviso, deshaciendo en silencio esta reparación. Una versión fijada se cachea de forma permanente y no puede rotar bajo nuestros pies: el contenido queda congelado, y volver a un origen vivo es un cambio de catálogo deliberado. El HostlistsRegistry de AdGuard hizo la misma sustitución para todas las listas de HaGeZi el 2026-08-10. Se consideró y se **rechazó** un espejo en GitLab con el mismo nombre de espacio: un nombre de usuario coincidente, la antigüedad del repositorio, los metadatos de los commits y las cabeceras copiadas son todos reproducibles por alguien ajeno, y ninguna firma ni enlace de primera parte lo autentica. |
