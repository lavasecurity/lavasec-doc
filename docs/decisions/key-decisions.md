...
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `gitlab.com/hagezi/mirror/-/raw/main/dns-blocklists` | GitHub locked the `hagezi` account, taking every list to a hard 404. The GitLab namespace `hagezi` is a **user** namespace whose owner is "Gerd" (HaGeZi's maintainer); the project was created 2026-04-08, months before the lock, and carries daily commits authored by `hagezi` (most recent 2026-08-12). All 11 files keep their upstream path and filename and serve their original `# Title: HaGeZi's …` headers. |
