...
**Rationale.** Treating Lava as a local filtering engine / user agent — rather than a blocklist distributor — minimizes GPLv3 redistribution and App Review exposure. The device fetches each list over TLS directly from its curated `source_url` and parses it locally under strict size/rule caps; community lists are accepted as served (the catalog's `accepted_source_hashes` are advisory, not a hard gate — a single pinned hash cannot track a fast-rotating upstream and only produced false rejections), while Lava's threat-guardrail tier stays hash-pinned. Provenance is enforced at the catalog, not by a client hash gate: a `source_url` change that puts a **different publisher** behind an existing entry must use a new `list_id`, so a device can never silently begin fetching another party's bytes under an identity the user already consented to. A move **between hosts within the same publisher's own namespace** keeps its `list_id` — the curator, the file names and the content are unchanged, only the CDN in front of them differs, and re-identifying such a move would retire every affected list on devices that already enabled it (the retired IDs are quarantined, so the user silently loses a list they chose). Same-publisher moves are recorded here instead of re-identified, with the evidence that established first-party ownership. Every parsed rule set is also passed through a protected-domain filter so an upstream list cannot block Lava/Apple/identity-provider domains. The model is enforced in CI by `check-gpl-blocklist-distribution.sh` (no mirror code, no Lava-hosted artifact URLs, no GPL sources default-enabled, no R2 byte writes).

**Recorded same-publisher moves.**

| Date | Lists | From → To | First-party evidence |
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@37522026.221.30748` | jsDelivr's `/gh/<owner>/<repo>` path resolves to that exact GitHub repository, so the publisher is fixed **by the URL itself** and no third party can be served under it — a CDN in front of the original source, not a different origin. GitHub has locked the `hagezi` account, so the raw host now 404s while jsDelivr keeps serving its cached copy of the same repo pinned to the **immutable** version `37522026.221.30748`, whose content is that of 2026-08-09. Not `@latest`: jsDelivr resolves that alias through the GitHub API, which already 404s for this repo, so it survives only on cached resolution and could stop resolving without warning, silently undoing this repair. A pinned version is cached permanently and cannot rotate under us — so the content is frozen, and moving back to a live origin is a deliberate catalog change. AdGuard's HostlistsRegistry made the same substitution for every HaGeZi list on 2026-08-10. A same-namespace GitLab mirror was considered and **rejected**: matching username, repository age, commit metadata and copied file headers are all reproducible by an unrelated party, and no signature or first-party cross-link authenticates it. |
