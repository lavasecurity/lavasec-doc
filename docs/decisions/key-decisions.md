...
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@latest` | jsDelivr's `/gh/<owner>/<repo>` path resolves to that exact GitHub repository, so the publisher is fixed **by the URL itself** and no third party can be served under it — a CDN in front of the original source, not a different origin. GitHub has locked the `hagezi` account, so the raw host now 404s while jsDelivr keeps serving its cached copy of the same repo (frozen at 2026-08-09; it resumes updating if the account is restored). AdGuard's HostlistsRegistry made the same substitution for every HaGeZi list on 2026-08-10. A same-namespace GitLab mirror was considered and **rejected**: matching username, repository age, commit metadata and copied file headers are all reproducible by an unrelated party, and no signature or first-party cross-link authenticates it. |
