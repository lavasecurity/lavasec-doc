...
**Begründung.** Der Pakettunnel ist der einzige Provider, der (a) auf nicht verwalteten Consumer-Geräten funktioniert und (b) jede DNS-Entscheidung direkt auf dem Gerät treffen lässt – das ist die Grundlage des Datenschutzversprechens: *Die gesamte DNS-Filterung passiert auf dem Gerät; Lava leitet dein Surfen nie über eigene Server und bekommt nie den Strom der Domains zu sehen, die du besuchst.* Der in Kauf genommene Kompromiss ist die iOS-Grenze von **~50 MiB Speicher pro Extension**, unter der der Tunnel bleiben muss – eine Einschränkung, die mehrere der weiter unten stehenden Entscheidungen prägt.
...
**Entscheidung.** Lava veröffentlicht nur die **URL der vorgelagerten Blockliste plus akzeptierte Hashes**; das Gerät holt sich die **Bytes** der Liste direkt von jeder `source_url` und parst, normalisiert, dedupliziert und filtert dann lokal. Lava speichert, spiegelt, transformiert und liefert Blocklisten-Bytes von Drittanbietern **niemals** aus. Der Worker schreibt nur **Metadaten**-JSON des Katalogs nach R2 (`raw_r2_key`/`normalized_r2_key` sind null).
...
**Begründung.** Lava als lokale Filter-Engine / User Agent zu behandeln – und nicht als Verteiler von Blocklisten – minimiert die Berührungspunkte mit der GPLv3-Weiterverteilung und mit dem App Review. Das Gerät holt jede Liste per TLS direkt von ihrer kuratierten `source_url` und parst sie lokal unter strengen Größen- und Regelgrenzen; Community-Listen werden so akzeptiert, wie sie geliefert werden (die `accepted_source_hashes` im Katalog sind beratend, keine harte Schranke — ein einzelner gepinnter Hash kann einen schnell rotierenden Upstream nicht verfolgen und erzeugte nur falsche Ablehnungen), während Lavas Threat-Guardrail-Stufe hash-gepinnt bleibt. Herkunft wird im Katalog erzwungen, nicht durch ein Client-Hash-Gate: Eine `source_url`-Änderung, die einen **anderen Herausgeber** hinter einen bestehenden Eintrag stellt, braucht eine neue `list_id`, damit ein Gerät niemals unbemerkt die Bytes einer anderen Partei unter einer bereits zugestimmten Identität lädt. Ein Wechsel **zwischen Hosts innerhalb des Namensraums desselben Herausgebers** behält seine `list_id` — Kurator, Dateinamen und Inhalt bleiben gleich, nur das CDN davor ändert sich, und eine Neu-Identifizierung würde jede betroffene Liste auf Geräten zurückziehen, die sie bereits aktiviert haben (zurückgezogene IDs landen in Quarantäne, die Nutzerin verliert also stillschweigend eine selbst gewählte Liste). Wechsel innerhalb desselben Herausgebers werden hier festgehalten statt neu identifiziert, samt Nachweis der Erstanbieter-Eigentümerschaft. Jeder geparste Regelsatz läuft außerdem durch einen Filter für geschützte Domains, damit eine vorgelagerte Liste keine Domains von Lava/Apple oder Identitätsanbietern blockieren kann. Das Modell wird in der CI durch `check-gpl-blocklist-distribution.sh` erzwungen (kein Spiegel-Code, keine von Lava gehosteten Artefakt-URLs, keine GPL-Quellen standardmäßig aktiv, keine Byte-Schreibvorgänge nach R2).

**Festgehaltene Wechsel innerhalb desselben Herausgebers.**

| Datum | Listen | Von → Nach | Erstanbieter-Nachweis |
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@37522026.221.30748` | Der Pfad `/gh/<owner>/<repo>` von jsDelivr löst genau auf dieses GitHub-Repository auf — der Herausgeber steht also **in der URL selbst** fest, und niemand Drittes kann darunter ausgeliefert werden: ein CDN vor der Originalquelle, kein anderer Ursprung. GitHub hat das Konto `hagezi` gesperrt, der Raw-Host liefert daher 404, während jsDelivr seine zwischengespeicherte Kopie desselben Repositorys weiter ausliefert auf die **unveränderliche** Version `37522026.221.30748` festgelegt, deren Inhalt dem Stand vom 2026-08-09 entspricht. Nicht `@latest`: jsDelivr löst diesen Alias über die GitHub-API auf, die für dieses Repository bereits 404 liefert, er lebt also nur von zwischengespeicherter Auflösung und könnte ohne Vorwarnung ausfallen und diese Reparatur stillschweigend rückgängig machen. Eine festgelegte Version wird dauerhaft zwischengespeichert und kann nicht unter uns ausgetauscht werden — der Inhalt ist damit eingefroren, und die Rückkehr zu einer aktiven Quelle ist eine bewusste Katalogänderung. AdGuards HostlistsRegistry hat am 2026-08-10 für jede HaGeZi-Liste dieselbe Umstellung vorgenommen. Ein GitLab-Spiegel im gleichnamigen Namensraum wurde geprüft und **verworfen**: übereinstimmender Benutzername, Projektalter, Commit-Metadaten und kopierte Dateikopfzeilen lassen sich alle von Unbeteiligten nachbilden, und keine Signatur und kein Erstanbieter-Querverweis weist ihn aus. |
...
Diese Entscheidungen sind kleiner, hatten aber jeweils eine festgehaltene Kehrtwende.
