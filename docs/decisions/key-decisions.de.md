...
**Begründung.** Lava als lokale Filter-Engine / User Agent zu behandeln – und nicht als Verteiler von Blocklisten – minimiert die Berührungspunkte mit der GPLv3-Weiterverteilung und mit dem App Review. Das Gerät holt jede Liste per TLS direkt von ihrer kuratierten `source_url` und parst sie lokal unter strengen Größen- und Regelgrenzen; Community-Listen werden so akzeptiert, wie sie geliefert werden (die `accepted_source_hashes` im Katalog sind beratend, keine harte Schranke — ein einzelner gepinnter Hash kann einen schnell rotierenden Upstream nicht verfolgen und erzeugte nur falsche Ablehnungen), während Lavas Threat-Guardrail-Stufe hash-gepinnt bleibt. Provenienz wird im Katalog erzwungen (eine `source_url`-Änderung braucht eine neue `list_id`), nicht durch ein Client-Hash-Gate. Jeder geparste Regelsatz läuft außerdem durch einen Filter für geschützte Domains, damit eine vorgelagerte Liste keine Domains von Lava/Apple oder Identitätsanbietern blockieren kann. Das Modell wird in der CI durch `check-gpl-blocklist-distribution.sh` erzwungen (kein Spiegel-Code, keine von Lava gehosteten Artefakt-URLs, keine GPL-Quellen standardmäßig aktiv, keine Byte-Schreibvorgänge nach R2).
