...
**Begründung.** Lava als lokale Filter-Engine / User Agent zu behandeln – und nicht als Verteiler von Blocklisten – minimiert die Berührungspunkte mit der GPLv3-Weiterverteilung und mit dem App Review. Das Gerät prüft die heruntergeladenen Bytes gegen die `accepted_source_hashes` des Katalogs und greift bei einer Abweichung auf den letzten funktionierenden Cache zurück oder schließt sicher ab (fail-closed), womit die Sicherheitseigenschaft wiederhergestellt wird, die zuvor die Spiegel-Pipeline geliefert hatte. Jeder geparste Regelsatz läuft außerdem durch einen Filter für geschützte Domains, damit eine vorgelagerte Liste keine Domains von Lava/Apple oder Identitätsanbietern blockieren kann. Das Modell wird in der CI durch `check-gpl-blocklist-distribution.sh` erzwungen (kein Spiegel-Code, keine von Lava gehosteten Artefakt-URLs, keine GPL-Quellen standardmäßig aktiv, keine Byte-Schreibvorgänge nach R2).
