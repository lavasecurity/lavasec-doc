...
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@latest` | Le chemin `/gh/<owner>/<repo>` de jsDelivr résout exactement vers ce dépôt GitHub : l'éditeur est donc fixé **par l'URL elle-même** et aucun tiers ne peut être servi dessous — un CDN devant la source d'origine, pas une autre origine. GitHub a verrouillé le compte `hagezi`, si bien que l'hôte raw renvoie 404 tandis que jsDelivr continue de servir sa copie en cache du même dépôt (figée au 2026-08-09 ; les mises à jour reprennent si le compte est rétabli). Le HostlistsRegistry d'AdGuard a effectué la même substitution pour toutes les listes HaGeZi le 2026-08-10. Un miroir GitLab dans un espace de noms homonyme a été examiné puis **rejeté** : nom d'utilisateur identique, ancienneté du dépôt, métadonnées de commits et en-têtes copiés sont tous reproductibles par un tiers, et aucune signature ni lien first-party ne l'authentifie. |
