...
**Logique.** Traiter Lava comme un moteur de filtrage local / agent utilisateur — plutôt que comme un distributeur de listes de blocage — minimise la redistribution sous GPLv3 et l'exposition à l'App Review. L'appareil récupère chaque liste via TLS directement depuis sa `source_url` sélectionnée et l'analyse localement sous des plafonds stricts de taille et de règles ; les listes communautaires sont acceptées telles que servies (les `accepted_source_hashes` du catalogue sont indicatifs, pas une barrière dure — un seul hachage épinglé ne peut pas suivre un amont qui tourne vite et ne faisait que produire de faux rejets), tandis que le niveau threat-guardrail de Lava reste épinglé par hachage. La provenance est imposée dans le catalogue, pas par une barrière de hachage côté client : un changement de `source_url` qui place un **éditeur différent** derrière une entrée existante doit utiliser un nouveau `list_id`, afin qu'un appareil ne commence jamais à télécharger en silence les octets d'un tiers sous une identité déjà acceptée. Un déplacement **entre hôtes au sein de l'espace de noms du même éditeur** conserve son `list_id` — le curateur, les noms de fichiers et le contenu sont inchangés, seul le CDN en façade diffère, et ré-identifier un tel déplacement retirerait chaque liste concernée sur les appareils qui l'avaient déjà activée (les identifiants retirés passent en quarantaine, la personne perd donc en silence une liste qu'elle avait choisie). Les déplacements au sein du même éditeur sont consignés ici plutôt que ré-identifiés, avec la preuve de propriété first-party. Chaque jeu de règles analysé passe aussi par un filtre de domaines protégés, pour qu'une liste en amont ne puisse pas bloquer les domaines de Lava/Apple/fournisseur d'identité. Le modèle est imposé en CI par `check-gpl-blocklist-distribution.sh` (pas de code de copie, pas d'URL d'artefact hébergé par Lava, aucune source GPL activée par défaut, aucune écriture d'octets dans R2).

**Déplacements consignés au sein du même éditeur.**

| Date | Listes | De → Vers | Preuve first-party |
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@37522026.221.30748` | Le chemin `/gh/<owner>/<repo>` de jsDelivr résout exactement vers ce dépôt GitHub : l'éditeur est donc fixé **par l'URL elle-même** et aucun tiers ne peut être servi dessous — un CDN devant la source d'origine, pas une autre origine. GitHub a verrouillé le compte `hagezi`, si bien que l'hôte raw renvoie 404 tandis que jsDelivr continue de servir sa copie en cache du même dépôt épinglée à la version **immuable** `37522026.221.30748`, dont le contenu est celui du 2026-08-09. Pas `@latest` : jsDelivr résout cet alias via l'API GitHub, qui renvoie déjà 404 pour ce dépôt, il ne tient donc qu'à une résolution en cache et pourrait cesser de fonctionner sans prévenir, annulant silencieusement cette réparation. Une version épinglée est mise en cache définitivement et ne peut pas changer sous nos pieds — le contenu est donc figé, et revenir à une origine vivante est un changement de catalogue délibéré. Le HostlistsRegistry d'AdGuard a effectué la même substitution pour toutes les listes HaGeZi le 2026-08-10. Un miroir GitLab dans un espace de noms homonyme a été examiné puis **rejeté** : nom d'utilisateur identique, ancienneté du dépôt, métadonnées de commits et en-têtes copiés sont tous reproductibles par un tiers, et aucune signature ni lien first-party ne l'authentifie. |
