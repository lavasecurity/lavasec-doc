...
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `gitlab.com/hagezi/mirror/-/raw/main/dns-blocklists` | GitHub a verrouillé le compte `hagezi`, mettant chaque liste en 404 franc. L'espace de noms GitLab `hagezi` est un espace **utilisateur** dont le propriétaire est « Gerd » (le mainteneur de HaGeZi) ; le projet a été créé le 2026-04-08, des mois avant le verrouillage, et porte des commits quotidiens signés `hagezi` (le dernier le 2026-08-12). Les 11 fichiers conservent leur chemin et leur nom et servent toujours leurs en-têtes d'origine `# Title: HaGeZi's …`. |
