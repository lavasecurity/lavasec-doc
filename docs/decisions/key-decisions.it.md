...
**Motivazione.** Trattare Lava come un motore di filtraggio locale / user agent — anziché come un distributore di blocklist — minimizza la ridistribuzione GPLv3 e l'esposizione in fase di App Review. Il dispositivo recupera ciascuna lista via TLS direttamente dal suo `source_url` curato e la analizza localmente sotto rigorosi limiti di dimensione/regole; le liste della community sono accettate così come servite (gli `accepted_source_hashes` del catalogo sono indicativi, non un cancello rigido — un singolo hash fissato non può seguire un upstream a rotazione rapida e produceva solo falsi rifiuti), mentre il tier threat-guardrail di Lava rimane vincolato all'hash. La provenienza è imposta a livello di catalogo, non da un cancello hash lato client: una modifica di `source_url` che mette un **editore diverso** dietro una voce esistente deve usare un nuovo `list_id`, così che un dispositivo non inizi mai in silenzio a scaricare i byte di un'altra parte sotto un'identità già accettata. Uno spostamento **tra host all'interno dello spazio dei nomi dello stesso editore** mantiene il proprio `list_id`: curatore, nomi dei file e contenuto restano invariati, cambia solo la CDN davanti, e ri-identificarlo ritirerebbe ogni lista interessata sui dispositivi che l'avevano già attivata (gli ID ritirati finiscono in quarantena, quindi la persona perde in silenzio una lista che aveva scelto). Gli spostamenti all'interno dello stesso editore sono registrati qui anziché ri-identificati, con la prova della proprietà first-party. Ogni set di regole analizzato passa anche attraverso un filtro di domini protetti affinché una lista upstream non possa bloccare i domini di Lava/Apple/del provider di identità. Il modello è imposto in CI da `check-gpl-blocklist-distribution.sh` (nessun codice di mirror, nessun URL di artefatto ospitato da Lava, nessuna sorgente GPL abilitata di default, nessuna scrittura di byte su R2).

**Spostamenti registrati all'interno dello stesso editore.**

| Data | Liste | Da → A | Prova first-party |
| --- | --- | --- | --- |
| 2026-08-13 | 11 `hagezi-*` | `raw.githubusercontent.com/hagezi/dns-blocklists` → `cdn.jsdelivr.net/gh/hagezi/dns-blocklists@37522026.221.30748` | Il percorso `/gh/<owner>/<repo>` di jsDelivr risolve esattamente a quel repository GitHub, quindi l'editore è fissato **dall'URL stesso** e nessun terzo può essere servito sotto di esso: una CDN davanti alla fonte originale, non un'origine diversa. GitHub ha bloccato l'account `hagezi`, così l'host raw ora risponde 404 mentre jsDelivr continua a servire la propria copia in cache dello stesso repository fissata alla versione **immutabile** `37522026.221.30748`, il cui contenuto è quello del 2026-08-09. Non `@latest`: jsDelivr risolve quell'alias tramite l'API di GitHub, che per questo repository dà già 404, quindi regge solo sulla risoluzione in cache e potrebbe smettere di risolvere senza preavviso, annullando in silenzio questa riparazione. Una versione fissata è memorizzata in modo permanente e non può cambiare sotto di noi: il contenuto resta congelato, e tornare a un'origine viva è una modifica di catalogo deliberata. L'HostlistsRegistry di AdGuard ha fatto la stessa sostituzione per ogni lista HaGeZi il 2026-08-10. Un mirror GitLab nello spazio dei nomi omonimo è stato valutato e **respinto**: nome utente coincidente, età del repository, metadati dei commit e intestazioni copiate sono tutti riproducibili da un estraneo, e nessuna firma né collegamento first-party lo autentica. |
