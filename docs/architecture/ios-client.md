...
This single assessment drives the in-app Guard surface in full; the Live Activity / Dynamic Island deliberately models only `on`/`paused`/`restarting` (see *How the two connect*, below) and does **not** mirror the connectivity severities, so the connectivity-problem states surface on the Guard tab but not on the Dynamic Island — a deliberate divergence, not a disagreement.
