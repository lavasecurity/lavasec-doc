...
Le Worker accède à Supabase via PostgREST (`/rest/v1`) et Auth (`/auth/v1`) avec un identifiant de rôle de service Supabase — il n'y a pas de SDK Supabase côté serveur ; les appels sont des `fetch` bruts via les helpers `supabase()` / `supabaseAuth()`.
...
La normalisation côté Worker (`normalizeBlocklist`) filtre les domaines protégés, applique des plafonds, et dédoublonne+trie. C'est purement pour calculer des métadonnées fiables ; pour les **listes communautaires**, l'appareil ne verrouille **pas** le téléchargement par hachage — il récupère via TLS depuis la `source_url` sélectionnée et analyse sous plafonds (les accepted hashes du catalogue sont indicatifs), donc cette normalisation côté Worker n'est pas une frontière de sécurité à elle seule. (Le niveau threat-guardrail de Lava reste épinglé par hachage sur l'appareil, et la provenance de la `source_url` est imposée à la publication — un changement d'URL qui place un **éditeur différent** derrière une entrée doit utiliser un nouveau `list_id` ; un changement d'hôte au sein de l'espace de noms du même éditeur conserve son `list_id` et est consigné dans la [décision clé 2](../decisions/key-decisions.md).) Constantes clés :
...
Les sources sélectionnées sont préchargées comme métadonnées source-url-only via des migrations, générées depuis le [Blocklist Catalog](../legal/blocklist-catalog.md) canonique (HaGeZi, OISD, The Block List Project, Phishing.Database, StevenBlack, AdGuard, 1Hosts). La migration d'extension des catégories ajoute les familles de défense en profondeur (nsfw/social/gambling/piracy), aligne le défaut de nouvelle installation sur **Block List Basic + StevenBlack Unified Hosts**, et réactive AdGuard DNS Filter comme option signalée pour revue juridique et désactivée par défaut. Statut : **Implémenté**.

> **Les valeurs par défaut du catalogue correspondent au client.** L'ensemble `default_enabled` du catalogue est désormais **{Block List Basic, StevenBlack Unified Hosts}** : des défauts source-url-only sous licence permissive, correspondant au défaut recommandé iOS (`AppConfiguration.lavaRecommendedDefaults`). La colonne `default_enabled` servie et le `DefaultCatalog` iOS embarqué sont générés depuis la même spécification canonique, donc ils correspondent par construction. À noter que `default_enabled` est informatif : la vraie barrière de niveau est le **quota de règles de filtrage (Gratuit 500 K / Plus 2 M)**, pas le nombre de listes. La justification juridique pour publier des URL (et pas des octets) est dans [Décision de conformité GPL source-url-only](../legal/gpl-source-url-only-compliance-decision.md).
