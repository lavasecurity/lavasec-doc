...
La normalisation côté Worker (`normalizeBlocklist`) filtre les domaines protégés, applique des plafonds, et dédoublonne+trie. C'est purement pour calculer des métadonnées fiables ; **l'appareil revalide les hachages acceptés** quand il télécharge la vraie liste, donc ce n'est pas une frontière de sécurité à elle seule. Constantes clés :
...
`isPublicBlocklistSource` ne publie une source que si son `status` est `sync` ou `nosync`, `redistribution_mode === "source_url_only"`, **et** `isAllowedLaunchGPLSource` passe. Le filtre GPL de lancement (`isAllowedLaunchGPLSource`) autorise librement les sources non-GPL mais restreint les sources GPL-3.0 aux préfixes de `list_id` `hagezi-` ou `oisd-`.

### 3.5 Sources préchargées et activées par défaut {#35-seeded-sources-default-enabled}

Les sources sélectionnées sont préchargées comme métadonnées source-url-only via des migrations (HaGeZi, OISD, Block List Project, Phishing.Database, AdGuard). La migration à faible risque (`20260526000000_low_risk_blocklist_sources.sql`) a d'abord préchargé `blocklistproject-basic` (Unlicense) avec `default_enabled = true`, forcé **toutes les sources GPL (HaGeZi/OISD) à `default_enabled = false`** en attendant l'avis du conseil juridique, et mis AdGuard DNS Filter en attente dans `license_review`. **Ce préchargement initial avec Basic par défaut a ensuite été remplacé** — la migration d'alignement ci-dessous bascule Basic à `false` et Phishing + Scam à `true` (le défaut actuellement servi). Statut : **Implémenté**.

> **Les valeurs par défaut du catalogue correspondent au client.** L'ensemble `default_enabled` du catalogue est désormais **{Block List Project Phishing, Block List Project Scam}**, ce qui correspond au défaut recommandé iOS (`AppConfiguration.lavaRecommendedDefaults`, dans `lavasec-ios: Sources/LavaSecCore/OnboardingDefaults.swift`). Une migration met `blocklistproject-basic default_enabled = false` et `blocklistproject-phishing` / `blocklistproject-scam default_enabled = true`, pour que les métadonnées servies soient justes. (la décision d'alignement est maintenant livrée.) À noter que `default_enabled` est informatif : la vraie barrière de niveau est le **quota de règles de filtrage (Gratuit 500 K / Plus 2 M)**, pas le nombre de listes. La justification juridique pour publier des URL (et pas des octets) est dans [Décision de conformité GPL source-url-only](../legal/gpl-source-url-only-compliance-decision.md).
