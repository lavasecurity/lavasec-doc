...
## 2. 세션 및 Keychain 저장 {#2-session--keychain-storage}
...
## 3. 제로 지식 백업 {#3-zero-knowledge-backup}

### 3.1 정확히 무엇인가 {#31-what-it-is-precisely}
...
### 3.2 무엇이 백업되는가 (최소화된 페이로드) {#32-what-gets-backed-up-the-minimized-payload}
...
### 3.3 봉투 (클라이언트 측 암호화) {#33-the-envelope-client-side-crypto}
...
### 3.4 업로드 및 저장 {#34-upload--storage}
...
### 3.5 보장 — 서버가 볼 수 있는 것과 볼 수 없는 것 {#35-the-guarantee--what-the-server-can-and-cannot-see}
...
### 4.2 보조 복구 (2요소 결합) {#42-assisted-recovery-the-two-factor-combination}
...
### 4.3 패스키 복구 — 제로 지식, PRF 파생 {#43-passkey-recovery--zero-knowledge-prf-derived}
...
## 5. 데이터 최소화 및 개인정보 자세 {#5-data-minimization--privacy-posture}
