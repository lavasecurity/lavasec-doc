...
## 1. La canalización de DNS (Implementado) {#1-the-dns-pipeline-implemented}
...
## 2. El motor de filtrado (Implementado) {#2-the-filtering-engine-implemented}

### 2.1 Precedencia de decisión {#21-decision-precedence}
...
### 2.2 Almacenamiento de reglas y la unidad de memoria residente {#22-rule-storage-and-the-resident-memory-unit}
...
### 2.3 Formas del snapshot compilado {#23-compiled-snapshot-forms}
...
## 3. Transportes cifrados y la ruta del resolutor (Implementado) {#3-encrypted-transports--the-resolver-path-implemented}
...
### 3.4 DoQ — conexión nueva por consulta {#34-doq--fresh-connection-per-query}
...
### 3.5 Política de resolución {#35-resolution-policy}
...
## 4. Presupuesto de reglas de filtrado, techo NE y mmap {#4-filter-rules-budget-ne-ceiling-and-mmap}
...
### 4.1 Límites por tier (Implementado) {#41-tier-limits-implemented}
...
### 4.2 Barrera de protección de memoria del dispositivo + techo NE (Implementado) {#42-device-memory-guardrail--ne-ceiling-implemented}
...
### 4.3 Estrategia mmap (Implementado) {#43-mmap-strategy-implemented}
...
### 4.4 Aplicación de dos capas (Implementado) {#44-two-layer-enforcement-implemented}
...
### 4.5 El analizador (Implementado) {#45-the-parser-implemented}
...
### 4.6 Robustez de descarga y decodificación (Implementado) {#46-download--decode-robustness-implemented}
...
## 5. Catálogo de listas de bloqueo y fuentes por defecto {#5-blocklist-catalog--default-sources}
...
### 5.2 Fuentes curadas (Implementado) {#52-curated-sources-implemented}
...
### 5.3 Listas activadas por defecto para usuarios gratuitos (Implementado) {#53-default-enabled-lists-for-free-users-implemented}
...
### 5.4 Modelo de distribución GPL basado solo en la URL de origen (Implementado) {#54-source-url-only-gpl-distribution-model-implemented}
...
## 6. Resumen de estado {#6-status-summary}
