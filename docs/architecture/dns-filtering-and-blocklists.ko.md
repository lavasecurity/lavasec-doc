...
## 1. DNS 파이프라인 (구현됨) {#1-the-dns-pipeline-implemented}
...
## 2. 필터링 엔진 (구현됨) {#2-the-filtering-engine-implemented}

### 2.1 결정 우선순위 {#21-decision-precedence}
...
### 2.2 규칙 저장과 상주 메모리 단위 {#22-rule-storage-and-the-resident-memory-unit}
...
### 2.3 컴파일된 스냅샷 형식 {#23-compiled-snapshot-forms}
...
## 3. 암호화 전송과 리졸버 경로 (구현됨) {#3-encrypted-transports--the-resolver-path-implemented}

### 3.1 전송 열거형 {#31-transport-enum}
...
### 3.4 DoQ — 쿼리마다 새 연결 {#34-doq--fresh-connection-per-query}
...
### 3.5 해석 정책 {#35-resolution-policy}
...
## 4. 필터 규칙 예산, NE 상한, 그리고 mmap {#4-filter-rules-budget-ne-ceiling-and-mmap}
...
### 4.1 등급 한도 (구현됨) {#41-tier-limits-implemented}
...
### 4.2 기기 메모리 가드레일 + NE 상한 (구현됨) {#42-device-memory-guardrail--ne-ceiling-implemented}
...
### 4.3 mmap 전략 (구현됨) {#43-mmap-strategy-implemented}
...
### 4.4 2층 적용 (구현됨) {#44-two-layer-enforcement-implemented}
...
### 4.5 파서 (구현됨) {#45-the-parser-implemented}
...
### 4.6 다운로드 및 디코드 견고성 (구현됨) {#46-download--decode-robustness-implemented}
...
## 5. 차단 목록 카탈로그 및 기본 소스 {#5-blocklist-catalog--default-sources}

### 5.1 카탈로그 모델 (구현됨) {#51-catalog-model-implemented}
...
### 5.2 큐레이션된 소스 (구현됨) {#52-curated-sources-implemented}
...
### 5.3 무료 사용자용 기본 활성화 목록 (구현됨) {#53-default-enabled-lists-for-free-users-implemented}
...
### 5.4 소스 URL 전용 GPL 배포 모델 (구현됨) {#54-source-url-only-gpl-distribution-model-implemented}
