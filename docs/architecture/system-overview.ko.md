...
## 1. 제품 한 줄 소개 {#1-product-one-liner}
...
## 2. 프라이버시 약속 (정본) {#2-the-privacy-promise-canonical}
...
## 3. 구성 요소 {#3-components}

### iOS 클라이언트 (실행 가능한 타깃 3개 + 공유 코드, 하나의 App Group `group.com.lavasec`) {#ios-client-three-executable-targets-shared-code-one-app-group-groupcomlavasec}
...
## 4. 데이터 흐름 다이어그램 {#4-data-flow-diagram}
...
## 5. 데이터 흐름 {#5-data-flows}

### A. DNS 경로 (쿼리마다, 전부 기기 내) — 구현됨 {#a-the-dns-path-per-query-all-on-device-implemented}
...
### B. 카탈로그 가져오기 + 차단목록 로드 (source-url 전용) — 구현됨 {#b-catalog-fetch-blocklist-load-source-url-only-implemented}
...
### C. 백업 (제로 지식, 옵트인) — 구현됨 {#c-backup-zero-knowledge-opt-in-implemented}
...
### D. 앱 ↔ 확장 제어 평면 — 구현됨 {#d-app-extension-control-plane-implemented}
...
## 6. 신뢰 경계 및 프라이버시 보존 설계 {#6-trust-boundaries-privacy-preserving-design}
...
## 7. 구성 요소별 문서 {#7-per-component-docs}
