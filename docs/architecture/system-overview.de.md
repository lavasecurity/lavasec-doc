...
> **Standard-aktiviert-Quelle der Wahrheit:** Der ausgelieferte kostenlose Standard ist **Block List Basic** (`OnboardingDefaults.lavaRecommendedDefaults`). Er wird auf dem Gerät aus dem `defaultEnabled`-Flag jeder kuratierten Quelle abgeleitet (`BlocklistSource.recommendedDefaultSourceIDs`), was die Spalte `default_enabled` im Backend-Katalog spiegelt, die aus derselben kanonischen Katalog-Spezifikation erzeugt wird.
