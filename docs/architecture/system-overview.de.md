...
3. Das Gerät parst die abgerufenen Bytes lokal unter Größen-/Regel-Obergrenzen. Community-Listen werden so akzeptiert, wie sie über TLS geliefert werden — die `accepted_source_hashes` des Katalogs sind beratend (Cache-Identität + Audit), keine harte Schranke — sodass eine rotierte Liste niemals abgelehnt wird, weil sie von einem festgepinnten Hash abweicht. Die Bedrohungs-Schutzbarriere-Stufe von Lava bleibt hash-festgepinnt.
...
> **Standard-aktiviert — Quelle der Wahrheit:** Der ausgelieferte kostenlose Standard ist **Block List Basic** (`OnboardingDefaults.lavaRecommendedDefaults`). Er wird auf dem Gerät aus dem `defaultEnabled`-Flag jeder geprüften Quelle abgeleitet (`BlocklistSource.recommendedDefaultSourceIDs`), was die Spalte `default_enabled` im Backend-Katalog spiegelt, die aus derselben kanonischen Katalogspezifikation generiert wird.
