...
Kuratierte Quellen werden per Migrationen als source-url-only-Metadaten vorbefüllt, generiert aus der kanonischen [Blocklisten-Katalog](../legal/blocklist-catalog.md)-Spezifikation (HaGeZi, OISD, The Block List Project, Phishing.Database, StevenBlack, AdGuard, 1Hosts). Die Migration zur Kategorie-Erweiterung fügt die Defensive-Depth-Kategorien (nsfw/social/gambling/piracy) hinzu, richtet den Standard für Neuinstallationen auf **Block List Basic** aus und reaktiviert den AdGuard DNS Filter als eine von der Rechtsabteilung markierte, standardmäßig deaktivierte Option. Status: **Umgesetzt**.

> **Die Katalog-Standards stimmen mit dem Client überein.** Das `default_enabled`-Set des Katalogs ist **{Block List Basic}** — eine breite, freizügige kombinierte Liste, die das frühere Paar Phishing + Scam ablöst — und passt damit zum empfohlenen iOS-Standard (`AppConfiguration.lavaRecommendedDefaults`). Sowohl die ausgelieferte `default_enabled`-Spalte als auch der gebündelte iOS-`DefaultCatalog` werden aus derselben kanonischen Spezifikation generiert, also stimmen sie konstruktionsbedingt überein (dies löst die frühere Standard-Diskrepanz zwischen Client und Backend). Beachte, dass `default_enabled` informativ ist: Das eigentliche Tier-Gate ist das **Filterregel-Budget (Free 500K / Plus 2M)**, nicht die Anzahl der Listen. Die rechtliche Begründung dafür, URLs (nicht Bytes) zu veröffentlichen, steht in [GPL-source-url-only-Compliance-Entscheidung](../legal/gpl-source-url-only-compliance-decision.md).
