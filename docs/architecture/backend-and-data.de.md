...
Die Worker-seitige Normalisierung (`normalizeBlocklist`) filtert geschützte Domains, erzwingt Obergrenzen und dedupliziert + sortiert. Das dient rein dazu, vertrauenswürdige Metadaten zu berechnen; bei **Community-Listen** hash-gatet das Gerät den Download **nicht** — es holt per TLS von der kuratierten `source_url` und parst unter Obergrenzen (die accepted hashes im Katalog sind beratend), also ist diese Worker-seitige Normalisierung für sich genommen keine Sicherheitsgrenze. (Lavas Threat-Guardrail-Stufe bleibt auf dem Gerät hash-gepinnt, und die Herkunft der `source_url` wird beim Veröffentlichen erzwungen — eine URL-Änderung, die einen anderen **Herausgeber** hinter einen Eintrag stellt, braucht eine neue `list_id`; ein Host-Wechsel innerhalb des Namensraums desselben Herausgebers behält seine `list_id` und wird in [Kernentscheidung 2](../decisions/key-decisions.md) festgehalten.) Wichtige Konstanten:
...
`isPublicBlocklistSource` veröffentlicht eine Quelle nur, wenn `status` `sync` oder `nosync` ist, `redistribution_mode === "source_url_only"` gilt **und** `isAllowedLaunchGPLSource` besteht. Das Launch-GPL-Gate (`isAllowedLaunchGPLSource`) lässt Nicht-GPL-Quellen frei zu und erlaubt die freigegebenen GPL-3.0-Quellfamilien per `list_id`-Präfix: `hagezi-`, `oisd-` und `adguard-`.

### 3.5 Vorbefüllte Quellen & default-enabled {#35-seeded-sources--default-enabled}

Kuratierte Quellen werden per Migrationen als source-url-only-Metadaten vorbefüllt und aus dem kanonischen [Blocklist Catalog](../legal/blocklist-catalog.md) erzeugt (HaGeZi, OISD, The Block List Project, Phishing.Database, StevenBlack, AdGuard, 1Hosts). Die Kategorie-Erweiterungsmigration ergänzt die Defense-in-Depth-Kategorien (nsfw/social/gambling/piracy), richtet den Fresh-Install-Standard auf **Block List Basic + StevenBlack Unified Hosts** aus und reaktiviert den AdGuard DNS Filter als rechtsgeprüft markierte, standardmäßig deaktivierte Option. Status: **Umgesetzt**.

> **Die Katalog-Standards stimmen mit dem Client überein.** Das `default_enabled`-Set des Katalogs ist jetzt **{Block List Basic, StevenBlack Unified Hosts}** — permissiv lizenzierte, source-url-only Standards, passend zum empfohlenen iOS-Standard (`AppConfiguration.lavaRecommendedDefaults`). Sowohl die ausgelieferte `default_enabled`-Spalte als auch der gebündelte iOS-`DefaultCatalog` werden aus derselben kanonischen Spezifikation erzeugt und stimmen dadurch konstruktionsbedingt überein. Beachte, dass `default_enabled` informativ ist: Das eigentliche Tier-Gate ist das **Filterregel-Budget (Free 500K / Plus 2M)**, nicht die Anzahl der Listen. Die rechtliche Begründung dafür, URLs (nicht Bytes) zu veröffentlichen, steht in [GPL-source-url-only-Compliance-Entscheidung](../legal/gpl-source-url-only-compliance-decision.md).
