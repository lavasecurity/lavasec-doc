...
3. L'appareil parse les octets récupérés en local, sous des plafonds de taille/de règles. Les listes communautaires sont acceptées telles que servies via TLS — les `accepted_source_hashes` du catalogue sont indicatifs (identité de cache + audit), pas une barrière stricte — ainsi une liste qui a tourné n'est jamais rejetée pour avoir dérivé d'un hachage épinglé. Le palier garde-fou de sécurité de Lava, lui, reste épinglé par hachage.
...
> **Source de vérité des valeurs activées par défaut :** le défaut gratuit livré est **Block List Basic** (`OnboardingDefaults.lavaRecommendedDefaults`). Il est dérivé sur l'appareil à partir du drapeau `defaultEnabled` de chaque source sélectionnée (`BlocklistSource.recommendedDefaultSourceIDs`), qui reflète la colonne `default_enabled` du catalogue backend générée à partir de la même spécification canonique de catalogue.
