...
- **Pause / reprise** passent par un verrou de fichier inter-processus (`protection-command.lock`, `flock`) et les `ProtectionPauseStore` / `ProtectionSessionStore` de `LavaSecCore`, qui possèdent l'attribution des révisions et la déduplication des commandes en double (le `commandID` reporte l'id d'opération de l'appelant pour qu'une commande re-livrée ne puisse pas créer une seconde révision). Le résultat planifie une mise à jour de Live Activity protégée par révision.
