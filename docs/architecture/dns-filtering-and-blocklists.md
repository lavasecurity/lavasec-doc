...
The actual free default config is `OnboardingDefaults.lavaRecommendedDefaults` (`Sources/LavaSecCore/OnboardingDefaults.swift`), which enables **Block List Project Phishing + Block List Project Scam**, with the device-DNS resolver preset (`resolverPresetID = DNSResolverPreset.device.id`). The encrypted Device-DNS fallback ships **on** in this default (`usesEncryptedDeviceDNSFallback = true`) and routes to **Mullvad DoH** (`fallbackResolverPresetID = DNSResolverPreset.mullvadDoH.id`): if the device's own DNS wedges, allowed lookups are carried transiently over Mullvad DoH and then return to the device's DNS automatically. (The bare `AppConfiguration()` initializer still defaults this fallback **off** — it is enabled only by accepting the recommended onboarding defaults.)
