...
Lava Security ist eine iOS-App, bei der Privatsphäre an erster Stelle steht. Sie filtert DNS direkt auf dem Gerät über einen geräteinternen NetworkExtension-Paket-Tunnel und blockiert bekannte riskante und unerwünschte Domains, ohne dass dein Surfen über die Server von Lava läuft. Dieses Dokument beschreibt, wie der iOS-Client aufgebaut ist: die Targets, wie die App mit ihrer Tunnel-Extension spricht, den VPN-Lebenszyklus, das Guardian-Zustandsmodell, die Live Activity und das Widget, den Onboarding-Ablauf und den app-seitigen Zustandsverwalter (`AppViewModel`).
...
| **Paket-Tunnel** (`LavaSecTunnel`) | `com.lavasec.app.tunnel` | Die `NEPacketTunnelProvider`-Unterklasse `PacketTunnelProvider` (auch `LavaSecTunnel` genannt). Sie analysiert DNS-Pakete, extrahiert die abgefragte Domain, prüft sie gegen den memory-mapped kompilierten Snapshot und leitet erlaubte Abfragen nach oben weiter. Begrenzt durch die jetsam-Speicherobergrenze von ~50 MiB pro Prozess. |
...
Die ausgelieferte Startkonfiguration kommt aus `OnboardingDefaults` (`Sources/LavaSecCore/OnboardingDefaults.swift`). `AppConfiguration.lavaRecommendedDefaults` aktiviert nur die permissiven empfohlenen Quellen (Block List Project Phishing + Scam), wählt **Device DNS** als Resolver — `DNSResolverPreset.device` (id `device-dns`), das eigene DNS des Netzwerks; verschlüsselte Presets wie Google DoH sind optional und werden nicht zum Standard erhoben —, aktiviert die Geräte-DNS-Ausweichoption und lässt das lokale Logging an — mit `protectionEnabled: false`, sodass der Schutz erst eingeschaltet wird, wenn der Nutzer es wählt. `OnboardingDefaultsSummary` formatiert diese Auswahl für die Anzeige ("Ohne Konto fortfahren" ist die Konto-Standardeinstellung).
