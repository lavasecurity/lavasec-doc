...
Die ausgelieferte Startkonfiguration kommt aus `OnboardingDefaults` (`Sources/LavaSecCore/OnboardingDefaults.swift`). `AppConfiguration.lavaRecommendedDefaults` aktiviert die permissiven empfohlenen Quellen (Block List Basic + StevenBlack Unified Hosts), wählt **Device DNS** als Resolver — `DNSResolverPreset.device` (id `device-dns`), das eigene DNS des Netzwerks; verschlüsselte Presets wie Google DoH sind optional und werden nicht zum Standard erhoben —, aktiviert die Geräte-DNS-Ausweichoption und lässt das lokale Logging an — mit `protectionEnabled: false`, sodass der Schutz erst eingeschaltet wird, wenn der Nutzer es wählt. `OnboardingDefaultsSummary` formatiert diese Auswahl für die Anzeige ("Ohne Konto fortfahren" ist die Konto-Standardeinstellung).
