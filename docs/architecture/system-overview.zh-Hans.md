...
3. 设备会在大小 / 规则上限内本地解析抓回来的字节。社区列表按 TLS 返回的内容接受——目录里的 `accepted_source_hashes` 是提示性数据（缓存身份 + 审计），不是硬门槛——所以快速轮换的列表不会因为偏离钉死的哈希而被拒绝。Lava 的 threat-guardrail 层仍保持哈希钉死。
...
> **默认启用的注意点（以代码为准）：** 出厂的免费默认值是 **Block List Basic + StevenBlack Unified Hosts**（`OnboardingDefaults.lavaRecommendedDefaults`）。它们是在设备上从每个精选源的 `defaultEnabled` 标志推导出来的（`BlocklistSource.recommendedDefaultSourceIDs`），这个标志是设备上的事实来源，且与后端目录的 `default_enabled` 列保持一致。
