...
Lava Security は、端末上の NetworkExtension パケットトンネルを通じて DNS を
端末内でローカルにフィルタリングする**プライバシー優先の iOS アプリ**です。
ブラウジングを Lava のサーバー経由にすることなく、既知の危険なドメインや
不要なドメインをブロックします。

!!! quote "プライバシーの約束"
    DNS フィルタリングは端末上でローカルに行われます。Lava があなたの日常的な
    DNS クエリやブラウジング履歴、ドメインごとのテレメトリを受け取ることはありません。
    そして任意のアカウントバックアップはエンドツーエンドで暗号化されているので、
    Lava が保管できるのは暗号文だけです。

このサイトは、Lava の仕組み、振る舞い、そしてその背後にある判断を説明する
公開マニュアルです。内容はオープンソースの
...
プラットフォーム間の振る舞いは[プラットフォーム間の対応状況](product/platform-parity.md)で
追跡しています。そこには安定した機能 id、各プラットフォームの状態、そして iOS と
Android の整合を保つためのテストやフィクスチャがまとめられています。
