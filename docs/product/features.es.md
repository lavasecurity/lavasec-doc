...
Lava Security es una app de iOS centrada en la privacidad que filtra DNS **localmente en el dispositivo** a través de un túnel de paquetes NetworkExtension, bloqueando dominios maliciosos y no deseados para usuarios no técnicos (padres, personas mayores) — con protección esencial gratis para siempre y sin necesidad de cuenta.
...
| **Arranque en frío con cierre seguro** | Free | Un túnel en frío sin instantánea reutilizable instala un `FailClosedRuntimeSnapshot` que bloquea todo el tráfico en lugar de filtrar DNS sin filtrar. |
...
| **Optimización de rendimiento** | Free | Activación con caché primero, fusión de consultas en curso, obtención en paralelo acotada y fusión de oscilaciones (activación en caliente medida en ~112 ms en iPhone 15 Pro según el trabajo de aceleración modular). |
...
| **Listas de bloqueo personalizadas** | Plus | `allowsCustomBlocklists`. Las listas personalizadas se obtienen y analizan en el dispositivo, se almacenan en caché localmente y nunca se enrutan a través de los servidores de Lava. |
...
| **Enrutamiento y conmutación por error del resolver** | Free | `ResolverOrchestrator` enruta por transporte, degrada a DNS plano cuando un plan cifrado no tiene endpoints, hace conmutación por error por endpoint con una barrera de retroceso, y luego recurre a device-DNS. |
...
| **Informe de errores** | Free | Asistente activado por el usuario que envía un paquete anonimizado a `POST /v1/bug-reports`; sin historial de dominios en v1. El paquete ahora también lleva la procedencia de la compilación (`appVersion`/`appBuild`/`sourceRevision`) y contadores de honestidad de conectividad. También accesible mediante agitar para informar (`RageShakeDetector`). |
