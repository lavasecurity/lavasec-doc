...
| **Listas de bloqueo predeterminadas gratis** | Free | Una instalación nueva habilita **Block List Basic** más **StevenBlack Unified Hosts** — valores por defecto source-url-only con licencias permisivas, marcados con `defaultEnabled: true` (`DefaultCatalog.recommendedDefaultSourceIDs`). Todo lo demás es opcional. |
...
| **Valores predeterminados recomendados aplicados** | Free | Resolver Device DNS, recurso a device-DNS activado, registro local activado (recuentos + historial + actividad), Block List Basic + StevenBlack Unified Hosts habilitadas, continuar sin cuenta (`lavasec-ios: Sources/LavaSecCore/AppConfiguration.swift`, `lavaRecommendedDefaults`). |
