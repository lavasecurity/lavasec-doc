...
| **Smart Save（仅在削弱防护时确认）** | 免费 | 对你的过滤所做的、只会*增强*防护或属于中性的改动（加一份拦截列表或一个已拦截域名）会直接生效；而会*削弱*防护的改动——移除一份拦截列表、移除一个已拦截域名，或者新增一个允许例外——则会先经过一张审阅确认页，并在新增例外时附带一块「请格外小心」的提示面板（`FiltersView.saveChanges()`，`weakensProtection`）。 |
...
| **活动 / 域名日志（防护详情）** | 免费 | 动态的、仅本地的诊断，从防护标签页进入（`GuardDestination.activity`）。摘要是一条请求**流**——一个「已处理请求」总数，拆成一条「允许／拦截」的流量条，并标注「本地保护占比」（诚实取整：占比极小时显示 `<1%`，接近全部时显示 `>99%`）。一个 **域名日志** 区块里放着 **热门域名**（拦截最多 & 允许最多，按查询次数排名）和 **域名历史**（最近的查询与判定）；只有在打开了历史记录的情况下，域名那几行才会出现。 |
| **过滤（防护详情）** | 免费 | 从防护标签页进入的单一、统一的过滤界面。一个「My filter」入口会打开一个整合到一起的 **My filter** 界面，里面有两层架子——**「Lava blocks these」**（拦截列表 + 单独被拦截的域名）和 **「Lava lets these through」**（允许例外）——共用同一套编辑／保存的草稿流程。一张「Phone → Lava → Internet」的流程图领头摆在这个标签页上，打开 My filter 时会自动刷新目录。 |
...
| **错误报告** | 免费 | 由用户触发的向导，把一份匿名化的数据包发到 `POST /v1/bug-reports`；v1 里不含域名历史。也可以摇一摇来报告（`RageShakeDetector`）。这份数据包现在还会带上构建来源信息（`appVersion`/`appBuild`/`sourceRevision`）和连通性诚实度计数器。 |
| **订阅管理** | Plus | 对活跃订阅用户，升级界面会显示「管理订阅」（自动续期方案，经由 `AppStore.showManageSubscriptions`）、「恢复购买」，以及权益到期日期；终身解锁则不显示「管理订阅」那一行。 |
...
- **LavaSecApp**（`com.lavasec.app`）—— SwiftUI 的 App 外壳；在这个构建里，根是一个两标签的 `TabView`（**防护** + **设置**），过滤和活动作为防护标签页下的详情页进入（网络活动现在住在设置 → 高级里）。
