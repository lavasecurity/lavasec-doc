...
Lava Security est une app iOS qui place la confidentialité avant tout : elle filtre le DNS **localement sur l'appareil** via un tunnel de paquets NetworkExtension, en bloquant les domaines malveillants et indésirables pour des utilisateurs non techniques (parents, personnes âgées) — avec une protection de base gratuite pour toujours et sans compte requis.
...
| **Pause temporaire (5 / 10 min) + reprise** | Free | La pause et la reprise passent par `LavaProtectionCommandService`, sous un verrou de fichier flock avec déduplication par révision. |
...
| **Optimisations de performance** | Free | Activation cache-first, regroupement des requêtes en cours, récupération en parallèle borné et regroupement des oscillations (activation à chaud mesurée à ~112 ms sur iPhone 15 Pro selon le travail de modularisation/accélération). |
...
| **Catalogue sélectionné (10 sources)** | Free, activable | `lavasec-ios: Sources/LavaSecCore/BlocklistModels.swift` (`DefaultCatalog.curatedSources`) : Block List Basic, Block List Project Phishing / Scam / Ransomware, Phishing.Database Active Domains, HaGeZi Multi Light / Normal / PRO mini / PRO, OISD Small. |
| **Listes de blocage par défaut (gratuites)** | Free | Une nouvelle installation active **Block List Project Phishing + Scam** (les deux sources marquées `defaultEnabled: true` ; `DefaultCatalog.recommendedDefaultSourceIDs`). |
| **Analyse / normalisation / déduplication sur l'appareil** | Free | `BlocklistParser` prend en charge auto/plain/hosts/adblock/dnsmasq, supprime les commentaires/lignes vides/entrées invalides, déduplique les chaînes exactes, et plafonne à 1 000 000 de règles par liste. |
| **Validation des octets en amont** | Free | Les octets récupérés passent par un SHA-256 et ne sont acceptés que si la somme de contrôle figure dans les `accepted_source_hashes` du catalogue ; en cas de non-correspondance, Lava revient au dernier cache valide ou se met en « fail-closed ». |
...
| **Exceptions autorisées (liste d'autorisation)** | Free | Liste d'autorisation gérée par l'utilisateur, qui permet certains domaines malgré les listes de blocage. Plafond Free : 10 domaines autorisés / 10 bloqués (`FeatureLimits.free`). |
...
| **Limites de domaines plus élevées** | Plus | 500 domaines autorisés / 500 bloqués (`FeatureLimits.plus`). |
...
| **Boutons d'action de la Live Activity** | Free | Pause 5 / 10 min, Reprise, Reconnecter — des `LiveActivityIntent` qui s'exécutent dans le processus de l'app via `LavaProtectionCommandService`. Les variantes de pause authentifiée exigent une authentification locale de l'appareil. |
...
| **Installation du profil VPN local** | Free | Installe la config du VPN local pendant l'onboarding **sans** activer Connect-On-Demand, pour que la protection ne soit jamais activée en silence à la fin — la surface Protection reste la référence. |
| **Demande d'autorisation de notifications** | Free | Demandée en cours de parcours, à l'étape notifications. |
| **Valeurs par défaut recommandées appliquées** | Free | Résolveur Device DNS, repli sur le DNS de l'appareil activé, journalisation locale activée (compteurs + historique + activité), Block List Project Phishing + Scam activées, continuer sans compte (`lavasec-ios: Sources/LavaSecCore/AppConfiguration.swift`, `lavaRecommendedDefaults`). |
...
| **Sélecteur d'apparence Lava Guard (7 apparences)** | Free | `GuardianShieldStyle` : `original, fireOpal, purpleObsidian, obsidian, cherryQuartz, emerald, kiwiCreme`, chacune avec une couleur de glyphe Dynamic Island assortie. |
...
| **Contrôles de journalisation locale uniquement** | Free | Bascules pour les compteurs de filtrage, l'historique des domaines (diagnostics) et l'activité réseau — tout est stocké sur l'appareil. |
| **Rapports / Activité (détail Protection)** | Free | Diagnostics dynamiques, locaux uniquement : compteurs de blocages/autorisations, santé du tunnel, principaux domaines. Les lignes de domaines n'apparaissent que si l'option historique est activée. Accessible comme écran de détail depuis l'onglet Protection (`GuardDestination.activity`). |
| **Filtres (détail Protection)** | Free | Écran de filtres orienté vue d'ensemble, avec le détail Domaines bloqués / Exceptions autorisées et un flux de brouillon par étapes voir/modifier/confirmer (`GuardDestination.filters`). |
| **Journal d'activité Réseau et État de Lava** | Free | Flux d'événements borné, local uniquement, des transitions réseau/runtime/utilisateur, partagé via l'App Group (`NetworkActivityLog`). |
| **Rapport de bug** | Free | Assistant déclenché par l'utilisateur, qui envoie un paquet anonymisé à `POST /v1/bug-reports` ; pas d'historique des domaines en v1. Aussi accessible en secouant l'appareil pour signaler (`RageShakeDetector`). |
...
- **LavaSecApp** (`com.lavasec.app`) — la coquille de l'app SwiftUI ; dans ce build, la racine est un `TabView` à deux onglets (**Protection** + **Réglages**), avec Filtres et Activité accessibles comme écrans de détail sous l'onglet Protection.
