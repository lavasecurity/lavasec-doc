...
| **Activité / Journaux de domaines (détail Protection)** | Free | Diagnostics dynamiques, locaux uniquement, accessibles depuis l'onglet Protection (`GuardDestination.activity`). Le digest est un **flux** de requêtes — un total de « requêtes traitées » réparti en une barre de volume Autorisées/Bloquées avec « % protégé localement » (arrondi honnête : une part infime affiche `<1 %`, une part quasi totale affiche `>99 %`). Une section **Journaux de domaines** contient les **Principaux domaines** (les plus bloqués et autorisés, classés par nombre de requêtes) et l'**Historique des domaines** (recherches et décisions récentes) ; les lignes de domaines n'apparaissent que si l'option historique est activée. |
| **Filtre (détail Protection)** | Free | Écran de filtre unifié unique, accessible depuis l'onglet Protection. Un hub « Mon filtre » ouvre un seul écran **Mon filtre** consolidé avec deux rayons — **« Lava bloque ceci »** (listes de blocage + domaines bloqués individuellement) et **« Lava laisse passer ceci »** (exceptions autorisées) — sous un même flux de brouillon Modifier/Enregistrer. Un schéma de flux « Téléphone → Lava → Internet » ouvre l'onglet, et ouvrir Mon filtre rafraîchit automatiquement le catalogue. |
...
| **Rapport de bug** | Free | Assistant déclenché par l'utilisateur, qui envoie un paquet anonymisé à `POST /v1/bug-reports` ; pas d'historique des domaines en v1. Aussi accessible en secouant l'appareil pour signaler (`RageShakeDetector`). Le paquet porte désormais aussi la provenance du build (`appVersion`/`appBuild`/`sourceRevision`) et des compteurs d'honnêteté de connectivité. |
| **Gestion de l'abonnement** | Plus | Pour les abonnés actifs, l'écran de mise à niveau affiche Gérer l'abonnement (offres à renouvellement automatique, via `AppStore.showManageSubscriptions`), Restaurer l'achat, et la date d'expiration du droit ; un déverrouillage à vie n'affiche aucune ligne Gérer. |
...
- **LavaSecApp** (`com.lavasec.app`) — la coquille de l'app SwiftUI ; dans ce build, la racine est un `TabView` à deux onglets (**Protection** + **Réglages**), avec Filtre et Activité accessibles comme écrans de détail sous l'onglet Protection (Activité réseau vit désormais sous Réglages → Avancé).
