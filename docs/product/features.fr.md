...
| **Catalogue sélectionné (catégorisé)** | Free, activable | Les sources sélectionnées sont organisées en catégories de défense en profondeur : Security & Threat Intel, Multi-purpose, Ads & Trackers, Social Media, Adult Content, Gambling, Piracy & Torrent. Elles viennent de HaGeZi, The Block List Project, OISD, StevenBlack, AdGuard, 1Hosts et Phishing.Database. L’ensemble actuel complet est publié dans le [Blocklist Catalog](../legal/blocklist-catalog.md). |
| **Listes de blocage par défaut (gratuites)** | Free | Une nouvelle installation active **Block List Basic** plus **StevenBlack Unified Hosts** — des défauts source-url-only sous licence permissive, marqués `defaultEnabled: true` (`DefaultCatalog.recommendedDefaultSourceIDs`). Tout le reste est optionnel. |
...
| **Intégrité amont (TLS + URL sélectionnée)** | Free | Les octets des listes communautaires sont récupérés via TLS directement depuis la `source_url` amont sélectionnée et acceptés sous plafonds de taille + format + nombre de règles ; les `accepted_source_hashes` du catalogue sont **indicatifs** (identité du cache + audit), pas une barrière dure — une liste qui tourne vite n'est jamais rejetée parce qu'elle a dérivé d'un hachage épinglé. Le niveau **threat-guardrail** de Lava (sélectionné par Lava, impossible à autoriser) reste strictement épinglé par hachage. |
