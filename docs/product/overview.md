...
- **Payment unlocks customization only — never baseline safety.** A server-signed list of known-dangerous domains (the threat guardrail) cannot be allowlisted by anyone, paid or not. Decision precedence is consistent everywhere: **threat guardrail > local allowlist (allowed exceptions) > blocklist > default-allow.** The tunnel ignores `isPaid`.
...
- **Curated blocklists, source-url-only** — Lava publishes only upstream list URLs plus accepted hashes; the device fetches, validates, and parses the list bytes itself, and Lava never mirrors or serves third-party blocklist bytes. The shipped default enables **Block List Project Phishing + Scam** (`AppConfiguration.lavaRecommendedDefaults`, defined in `OnboardingDefaults.swift`); GPL sources (HaGeZi, OISD) are opt-in. See [DNS filtering and blocklists](../architecture/dns-filtering-and-blocklists.md).
...
- **Optional accounts and backup** — Apple or Google sign-in with an end-to-end-encrypted ([zero-knowledge](../architecture/accounts-and-backup.md)) settings backup and recovery phrase; account deletion is self-serve. Server-gated passkey recovery is **(Planned)** — server-side WebAuthn verification is not yet shipped, and recovery is server-gated, not zero-knowledge. Accounts are optional; protection works fully signed-out.
