...
- **Curated blocklists** — a source-url-only catalog of curated lists with always-on guardrails; the shipped default enables only the permissive Block List Basic, and GPL sources (HaGeZi, OISD) are opt-in. See [DNS filtering and blocklists](../architecture/dns-filtering-and-blocklists.md).
