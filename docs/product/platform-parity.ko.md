...
| `filtering.guardrail-precedence` | 상시 가드레일은 사용자 허용 목록을 무시합니다; 유료 상태는 가드레일을 절대 우회하지 않습니다. | Shipped | Planned | 정확한 동작 패리티. | `CompactFilterSnapshotTests`; 이식 시 Android `FilterSnapshotTest`. |
...
| `filtering.on-device-parsing` | 선택된 목록은 기기에서 가져와 파싱됩니다; 일상적인 도메인 기록은 Lava에 업로드되지 않습니다. | Shipped | Planned | 정확한 개인정보 보호 패리티, 네이티브 저장소 허용. | `BlocklistParserTests`; 이식 시 Android 파서 패리티 테스트. |
...
| `account.optional-sign-in` | 보호 기능은 계정 없이 작동합니다; 로그인은 선택 사항입니다. | Shipped | Deferred | Android가 계정 기능을 노출하기 전 정확한 제품 약속. | 계정 인증 문서; Android 온보딩/설정 리뷰. |
| `backup.zero-knowledge-settings` | 선택적 설정 백업은 암호문만 저장합니다; Lava는 평문 백업 내용을 읽을 수 없습니다. | Shipped | Deferred | Android가 백업을 제공하기 전 정확한 개인정보 보호 패리티. | 제로 지식 백업 테스트; 구축 시 Android 암호화 패리티 테스트. |
| `plus.customization-boundary` | 무료 보호 기능은 계속 유용합니다; Plus는 고급 커스터마이징을 잠금 해제하며 가드레일 안전성을 절대 변경하지 않습니다. | Shipped | Planned | 동일한 제품 경계; 스토어 구현은 플랫폼 네이티브임. | 구독 정책 테스트; 구축 시 Play Billing 권한 테스트. |
...
- 개인정보 보호를 수반하는 모든 기능에는 기능 id가 있다;
- 정확한 패리티 동작에는 식별된 iOS 테스트 또는 픽스처 소스가 있다;
- 플랫폼 네이티브 동작에는 명시적인 Android 입장이 있다;
- 지연된 기능은 명명되어 Android MVP가 그것이 출시된다고 실수로
  암시하지 않게 한다.
