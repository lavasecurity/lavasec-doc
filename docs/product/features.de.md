...
Lava Security ist eine iOS-App mit Privatsphäre an erster Stelle. Sie filtert DNS **lokal auf dem Gerät** über einen NetworkExtension-Pakettunnel und blockiert schädliche und unerwünschte Domains für technisch weniger versierte Nutzer (Eltern, ältere Menschen). Der Grundschutz ist dauerhaft kostenlos und erfordert kein Konto.
...
| **Vorübergehende Pause (konfigurierbar 1–30 Min., Standard 5) + Fortsetzen** | Free | Pause/Fortsetzen laufen über `LavaProtectionCommandService` unter einer flock-Dateisperre mit Revisions-Dedup. |
...
| **Soft-Shield-Zustandsmodell von Lava** | Free | 7 Ausdruckszustände — `sleeping, waking, awake, paused, retrying, concerned, grateful` (`GuardianMascotAnimation.swift`, LavaSecCore). 6 Verbindungs-Schweregrade fallen zu 4 Gesichtern zusammen; in der App, im Onboarding und in der Live Activity identisch dargestellt. |
...
| **Performance-Härtung** | Free | Cache-first beim Einschalten, Zusammenführen laufender Anfragen, begrenzt paralleles Abrufen und Flap-Coalescing (warmes Einschalten laut der modularen Beschleunigungsarbeit mit ~112 ms auf dem iPhone 15 Pro gemessen). |
...
| **Kuratierter Katalog (kategorisiert)** | Free aktivierbar | Kuratierte Quellen, organisiert in Kategorien der Verteidigungstiefe — Security & Threat Intel, Multi-purpose, Ads & Trackers, Social Media, Adult Content, Gambling, Piracy & Torrent — von HaGeZi, The Block List Project, OISD, StevenBlack, AdGuard, 1Hosts und Phishing.Database. Der vollständige, aktuelle Satz ist im [Blocklist Catalog](../legal/blocklist-catalog.md) veröffentlicht; jede Plattform spiegelt die Katalogversion wider, mit der sie ausgeliefert wurde. |
| **Kostenlose Standard-Blocklisten** | Free | Eine frische Installation aktiviert **Block List Basic** — eine breite, freizügige kombinierte Liste (die mit `defaultEnabled: true` gekennzeichnete Quelle; `DefaultCatalog.recommendedDefaultSourceIDs`). Alles andere ist optional zuschaltbar. |
...
| **Upstream-Integrität (TLS + kuratierte URL)** | Free | Die Bytes von Community-Listen werden über TLS direkt von der kuratierten Upstream-`source_url` geholt und akzeptiert, sofern sie die Grenzen für Größe + Format + Regelanzahl einhalten; die `accepted_source_hashes` des Katalogs sind **beratend** (Cache-Identität + Audit), kein hartes Gatter — eine sich schnell ändernde Liste wird nie abgelehnt, nur weil sie von einem fest hinterlegten Hash abweicht. Lavas **Schutzbarriere**-Stufe (von Lava kuratiert, kann nicht erlaubt werden) bleibt strikt hash-gepinnt. |
| **Filter für geschützte Domains** | Free | Aus jeder geparsten Quelle werden geschützte Lava-/Apple-/Identitätsanbieter-Domains entfernt (apple.com, icloud.com, lavasecurity.app, google.com, accounts.google.com, …), damit eine Upstream-Liste die App, den Tunnel oder die Anmeldung nicht stören kann. |
...
| **Resolver-Routing & Failover** | Free | `ResolverOrchestrator` routet nach Transport, fällt auf einfaches DNS zurück, wenn ein verschlüsselter Plan keine Endpunkte hat, macht Failover pro Endpunkt mit einer Backoff-Sperre und greift dann auf die Geräte-DNS-Ausweichoption zurück. |
...
| **Schutzanzeige mit 5 Zuständen** | Free | `ProtectionState`: `on, paused, reconnecting, needsReconnect, networkUnavailable` — jeder ist einer Pose von Lava, einem SF Symbol und einem Titel zugeordnet. |
| **Aktions-Buttons der Live Activity** | Free | Pause für N Min. (konfigurierte Länge, Standard 5), Fortsetzen, Neu verbinden — `LiveActivityIntent`s, die im App-Prozess über `LavaProtectionCommandService` laufen. Die authentifizierten Pausen-Varianten erfordern lokale Geräte-Authentifizierung. |
...
| **Empfohlene Standardwerte angewendet** | Free | Device-DNS-Resolver, Geräte-DNS-Ausweichoption an, lokales Logging an (Zähler + Verlauf + Aktivität), Block List Basic aktiviert, ohne Konto fortfahren (`lavasec-ios: Sources/LavaSecCore/AppConfiguration.swift`, `lavaRecommendedDefaults`). |
...
| **Steuerung für rein lokales Logging** | Free | Schalter für Filterzähler, Domainverlauf (Diagnose) und Netzwerkaktivität — alles auf dem Gerät gespeichert. Feingranulare Logs (Domainverlauf + Netzwerkaktivität) werden auf ein **7-Tage**-Fenster gekürzt (`LocalLogRetention.fineGrainedDays = 7`); Zähler und Lava-Guard-Fortschritt werden länger aufbewahrt. |
...
| **Fehlerbericht** | Free | Vom Nutzer ausgelöster Assistent, der ein anonymisiertes Paket an `POST /v1/bug-reports` schickt; kein Domainverlauf in v1. Das Paket trägt jetzt außerdem die Build-Provenienz (`appVersion`/`appBuild`/`sourceRevision`) und Zähler zur Konnektivitäts-Ehrlichkeit. Auch per Schütteln-zum-Melden erreichbar (`RageShakeDetector`). |
| **Abo-Verwaltung** | Plus | Für aktive Abonnenten zeigt der Upgrade-Bildschirm Abo verwalten (automatisch verlängernde Tarife, über `AppStore.showManageSubscriptions`), Kauf wiederherstellen und das Ablaufdatum der Berechtigung. |
...
- **LavaSecApp** (`com.lavasec.app`) — die SwiftUI-App-Hülle; in diesem Build ist die Wurzel eine `TabView` mit zwei Tabs (**Guard** + **Settings**), wobei Filter und Aktivität als Detailbildschirme unter dem Guard-Tab erreichbar sind (Netzwerkaktivität lebt jetzt unter Einstellungen → Erweitert).
