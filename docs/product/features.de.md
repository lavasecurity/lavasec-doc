...
| **Kuratierter Katalog (kategorisiert)** | Free aktivierbar | Kuratierte Quellen sind in Defense-in-Depth-Kategorien organisiert: Security & Threat Intel, Multi-purpose, Ads & Trackers, Social Media, Adult Content, Gambling, Piracy & Torrent. Quellen stammen von HaGeZi, The Block List Project, OISD, StevenBlack, AdGuard, 1Hosts und Phishing.Database. Der vollständige aktuelle Satz steht im [Blocklist Catalog](../legal/blocklist-catalog.md). |
| **Kostenlose Standard-Blocklisten** | Free | Eine frische Installation aktiviert **Block List Basic** plus **StevenBlack Unified Hosts** — permissiv lizenzierte source-url-only Standards mit `defaultEnabled: true` (`DefaultCatalog.recommendedDefaultSourceIDs`). Alles andere ist opt-in. |
...
| **Upstream-Integrität (TLS + kuratierte URL)** | Free | Community-Listen-Bytes werden direkt per TLS von der kuratierten Upstream-`source_url` geholt und unter Größen-, Format- und Regelanzahlgrenzen akzeptiert; die `accepted_source_hashes` im Katalog sind **beratend** (Cache-Identität + Audit), keine harte Schranke — eine schnell rotierende Liste wird nie abgelehnt, nur weil sie von einem gepinnten Hash abweicht. Lavas **Threat-Guardrail**-Stufe (Lava-kuratiert, darf nicht erlaubt werden) bleibt streng hash-gepinnt. |
...
| **Empfohlene Standardwerte angewendet** | Free | Device-DNS-Resolver, Geräte-DNS-Ausweichoption an, lokales Logging an (Zähler + Verlauf + Aktivität), Block List Basic + StevenBlack Unified Hosts aktiviert, ohne Konto fortfahren (`lavasec-ios: Sources/LavaSecCore/AppConfiguration.swift`, `lavaRecommendedDefaults`). |
