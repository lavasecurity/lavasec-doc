...
Lava Security 的平台一致性系統會追蹤哪些產品承諾在 iOS、Android 以及未來的用戶端之間共享。它是功能行為的公開合約：什麼必須在各處代表相同的意義、什麼是刻意採用平台原生的做法，以及什麼尚未承諾。
...
|---|---|---:|---:|---|---|
| `protection.local-dns-filtering` | Lava Security 會在裝置本機上篩選 DNS，且不會透過 Lava Security 伺服器代理瀏覽流量。 | Shipped | Planned | 依意圖一致；OS 通道 API 各異。 | iOS 封包通道架構；Android `VpnService` 計畫。 |
...
| `filtering.source-url-only-catalog` | Lava Security 發佈的是目錄中繼資料與上游來源 URL，而非第三方封鎖清單的位元組內容。 | Shipped | Planned | 完全一致的隱私／智財模型。 | 目錄架構；GPL／僅來源 URL 的法律文件。 |
| `filtering.on-device-parsing` | 選定的清單會在裝置上擷取與解析；日常的網域歷史記錄不會上傳到 Lava Security。 | Shipped | Planned | 完全一致的隱私；允許原生儲存。 | `BlocklistParserTests`；移植後的 Android 解析器一致性測試。 |
...
| `dns.built-in-resolvers` | 使用者可以選擇內建的解析器預設組合，而不會將允許的查詢送到 Lava Security。 | Shipped | Planned | 相同的解析器政策；預設組合可能分階段推出。 | 解析器預設測試；移植後的 Android 解析器 DTO 測試。 |
...
| `backup.zero-knowledge-settings` | 選用的設定備份僅儲存密文；Lava Security 無法讀取備份內容的明文。 | Shipped | Deferred | 在 Android 提供備份之前，需有完全一致的隱私。 | 零知識備份測試；建置後的 Android 加密一致性測試。 |
| `plus.customization-boundary` | 免費防護仍然實用；Lava Security Plus 解鎖進階自訂功能，且絕不改變安全防護欄的安全性。 | Shipped | Planned | 相同的產品界線；商店實作為平台原生。 | 訂閱政策測試；建置後的 Play Billing 權益測試。 |
