...
- **Kuratierte Blocklisten, nur über Quell-URLs** — Lava veröffentlicht nur die URLs vorgelagerter Listen plus beratende Hashes für Cache-Identität und Audit; das Gerät holt jede Liste per TLS und wertet sie lokal unter Größen- und Regelgrenzen aus, und Lava spiegelt oder liefert nie die Blocklisten-Bytes Dritter. Community-Listen sind nicht hash-gepinnt — TLS plus kuratierte URL bilden die Integritätsgrenze — während Lavas Threat-Guardrail-Stufe weiter Hashes erzwingt. Der ausgelieferte Standard aktiviert **Block List Basic + StevenBlack Unified Hosts** (`AppConfiguration.lavaRecommendedDefaults`, definiert in `OnboardingDefaults.swift`); Copyleft-Quellen wie HaGeZi, OISD, AdGuard und 1Hosts sind optional zuschaltbar. Siehe [DNS-Filterung und Blocklisten](../architecture/dns-filtering-and-blocklists.md).
