...
Willkommen bei Lava Security. Diese Seite stellt vor, was Lava ist, was es verspricht und wo du mehr nachlesen kannst.
...
iOS bezeichnet das als "VPN", weil ein Pakettunnel die einzige Möglichkeit für eine App ist, DNS systemweit zu filtern — aber Lava ist **DNS-/Blocklisten-Filterung**, kein Umleiten von Datenverkehr. Sei ehrlich, was den Umfang angeht: Lava filtert lokal DNS-Domains und ist **keine** Garantie dafür, dass wirklich jede schädliche Domain oder URL blockiert wird. Lava sieht Domains, nicht einzelne Seitenpfade, und kann deshalb keine einzelne schlechte Seite auf einem ansonsten vertrauenswürdigen Host blockieren. Der Schutz ist außerdem nicht automatisch an, sobald die Einrichtung fertig ist — der **Guard**-Tab in der App ist die maßgebliche Stelle, an der du siehst, ob der Schutz gerade aktiv ist.
...
Dieser Satz ist verbindlich; alles andere in dieser Doku ist damit konsistent. Wenn du für die optionale Stufe bezahlst, wandert das Filtern **nicht** auf den Server und Lava bekommt auch keinen Strom deiner besuchten Domains. Wenn eine Funktion einen Server berührt, schreibt die Doku ausdrücklich auf, was **nicht** gesendet wird — deine alltäglichen DNS-Anfragen, dein Browserverlauf und jeglicher Klartext bleiben alle auf dem Gerät. Das ganze Bild findest du unter [Backend und Datenmodell](../architecture/backend-and-data.md).
...
- **Ruhiger Kern, verdiente Tiefe.** Die Standardoberflächen sind leise und beruhigend, angeführt vom Maskottchen Soft Shield Guardian und Texten, die auf angstmachende Sprache verzichten. Reichere, technischere Details gibt es, wenn du sie suchst, aber sie werden dir nie aufgedrängt. Diese Philosophie "ruhiger Kern, verdiente Tiefe" ist im **LavaTier**-Tiefenmodell (Floor / Window / Workshop) festgehalten — siehe [das Designsystem](../design-system/overview.md).
...
- **Kuratierte Blocklisten, nur über Quell-URLs** — Lava veröffentlicht nur die URLs vorgelagerter Listen (plus beratende Hashes für Cache-Identität und Audit); das Gerät holt jede Liste über TLS und wertet sie lokal unter Größen-/Regel-Obergrenzen aus, und Lava spiegelt oder liefert nie die Blocklisten-Bytes Dritter. Community-Listen sind nicht per Hash gepinnt — TLS + die kuratierte URL ist die Integritätsgrenze —, während die Schutzbarriere-Stufe von Lava per Hash durchgesetzt bleibt. Der ausgelieferte Standard aktiviert **Block List Basic** (`AppConfiguration.lavaRecommendedDefaults`, definiert in `OnboardingDefaults.swift`); Copyleft-Quellen wie HaGeZi, OISD, AdGuard und 1Hosts sind optional zuschaltbar. Siehe [DNS-Filterung und Blocklisten](../architecture/dns-filtering-and-blocklists.md).
...
- **Der Soft Shield Guardian** — ein Maskottchen im Guard-Tab, in der Live Activity und in der Dynamic Island, die den Schutzzustand über 7 Ausdruckszustände zeigt. Siehe [das Designsystem](../design-system/overview.md).
...
- **Optionale Konten und Backup** — Anmeldung mit Apple oder Google samt Ende-zu-Ende-verschlüsseltem ([Zero-Knowledge](../architecture/accounts-and-backup.md)) Einstellungs-Backup und Wiederherstellungscode; die Kontolöschung ist selbst durchführbar. Der optionale Passkey-Wiederherstellungsplatz ist **ebenfalls Zero-Knowledge** — sein Schlüssel wird auf dem Gerät aus der WebAuthn-PRF des Authentifikators abgeleitet, ohne ein vom Server gehaltenes Geheimnis; ob das auf dem Gerät produktiv einsatzbereit ist, hängt noch vom Hosting der Associated Domains / AASA ab **(Geplant)**. Konten sind optional; der Schutz funktioniert auch komplett abgemeldet.
