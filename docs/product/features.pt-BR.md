...
| **Blocklists padrão gratuitas** | Free | Uma instalação nova habilita o **Block List Basic** mais **StevenBlack Unified Hosts** — padrões source-url-only com licenças permissivas marcados com `defaultEnabled: true` (`DefaultCatalog.recommendedDefaultSourceIDs`). Todo o resto é opt-in. |
...
| **Padrões recomendados aplicados** | Free | Resolver Device DNS, fallback para device-DNS ativado, logging local ativado (contagens + histórico + atividade), Block List Basic + StevenBlack Unified Hosts habilitados, continuar sem conta (`lavasec-ios: Sources/LavaSecCore/AppConfiguration.swift`, `lavaRecommendedDefaults`). |
