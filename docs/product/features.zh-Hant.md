...
| **智慧儲存（僅在削弱時確認）** | 免費 | 對篩選器僅*強化*或屬中性的編輯（新增封鎖清單或封鎖網域）會直接套用；而*削弱*防護的編輯——移除封鎖清單、移除封鎖網域，或新增允許例外——則會先經過一張審查確認表單，並在新增例外時顯示「請格外小心」面板（`FiltersView.saveChanges()`、`weakensProtection`）。 |
...
| **Lava Guard 外觀選擇器（7 種外觀）** | 免費 | `GuardianShieldStyle`：`original, fireOpal, purpleObsidian, obsidian, cherryQuartz, emerald, kiwiCreme`，每一種都搭配一個動態島字符顏色。透過底部表單的單選選擇器選取（「選擇你的 Guard」，`LavaGuardLookPickerSheet`）；仍受限的外觀會帶有鎖頭字符，解鎖／升級面板則位於該表單內。 |
| **比對應用程式圖示** | 免費 | 可選的替代應用程式圖示，搭配所選的 guardian 外觀。 |
| **外觀** | 免費 | 淺色／深色／系統配色方案。 |
| **僅本機的記錄控制** | 免費 | 篩選計數、網域歷史記錄（診斷）與網路活動的開關——皆儲存於裝置上。細粒度記錄（網域歷史記錄＋網路活動）會被修剪至 **7 天**的範圍（`LocalLogRetention.fineGrainedDays = 7`）；計數與 Lava Guard 進度則保留更久。 |
| **活動／網域記錄（Guard 詳細）** | 免費 | 動態的僅本機診斷，從 Guard 分頁進入（`GuardDestination.activity`）。摘要是一條請求**流程**——「已處理的請求」總數拆分為允許／封鎖量的長條，並附上「於本機保護的 %」（誠實取整：極小比例顯示為 `<1%`，近乎全部則顯示為 `>99%`）。一個**網域記錄**區段含有 **Top Domains**（最常封鎖與允許者，依查詢數排序）與**網域歷史記錄**（近期查詢與決策）；網域列僅在歷史記錄選擇加入開啟時出現。 |
