...
> 対象読者: PM / エンジニア。このカタログで扱うのは、**現在実装済みの機能のみ**です。設計済みで未実装のものは、ここではなく非公開のロードマップに記載しています。

Lava Security は、プライバシーを最優先に考えた iOS アプリです。NetworkExtension のパケットトンネルを通して、**端末の中だけで**ローカルに DNS をフィルタリングし、悪意のあるドメインや邪魔なドメインをブロックします。技術に詳しくないユーザー（保護者や高齢の方など）でも使えるよう設計されており、基本の保護は永続的に無料で、アカウントも不要です。

以下のどの機能にも、次のプライバシーの約束があります。

> DNS のフィルタリングはすべて端末上で行われます。Lava がユーザーのブラウジングを自社サーバー経由で中継することはなく、アクセスしたドメインの記録を受け取ることもありません。バックエンドが保持するのは、カタログのメタデータ、ユーザーごとの不透明な暗号化バックアップ、そしてユーザーが送信を選択した匿名化済みの診断データのみです。
...
製品の中核です。ローカルで DNS のみを扱うパケットトンネルと、その周囲の穏やかな状態モデルです。
...
| **Connect-On-Demand** | 無料 | `NEOnDemandRuleConnect` が保護を維持し、自動で再起動します。接続が確認された**後にのみ**有効になり、プロファイルのインストール時には有効になりません。オンボーディング未完了の間は無効化されるため、インストール直後のアプリが、オフにできないトンネルを勝手に立ち上げることはありません。 |
| **一時停止（1〜30 分で設定可能、デフォルト 5 分）と再開** | 無料 | 一時停止と再開は `LavaProtectionCommandService` を通り、flock のファイルロック下でリビジョンの重複排除をしながら動きます。 |
| **認証が必要な一時停止** | 無料 | サーフェスごとにオプトインで設定できるゲートです（`SecurityProtectedSurface.protectionPause`）。一時停止には端末のローカル認証が必要で、認証なしの一時停止はコマンドサービスが拒否し、Live Activity は一時停止ボタンを隠します。 |
...
| **パフォーマンスの強化** | 無料 | キャッシュ優先のオン、処理中クエリの結合、上限付き並列フェッチ、フラップの結合（ウォーム状態でのオンは、モジュール化による高速化作業において iPhone 15 Pro 上で約 112 ms を計測）。 |
...
| **予算メーター（保存可能な選択）** | 無料 / Plus | 選択メーターはカウントを略記し（500K / 1.2M / 2M）、1.10 のソフト上限マージンを用います（リストごとの合計は、重複排除後の和集合を約 7〜10% 多めに見積もります）。許容範囲内に収まっているカウントは、ソフト上限を超えるまで、たとえば「500K of 500K」と表示されるよう丸められます（`FilterRuleBudget`）。 |
...
ブロックされなかったクエリに対する、リゾルバーのトランスポートとルーティング。
...
| **デバイス DNS フォールバック** | 無料 | 選択したリゾルバーが利用できないとき、現在接続中のネットワークのリゾルバーにフォールバックします。**デフォルトでオン**。`usingDeviceDNSFallback` の深刻度として表示されます。 |
...
| 機能 | プラン | 補足 |
|---|---|---|
| **任意のアカウントログイン（Apple + Google）** | 無料 | ネイティブの id_token フローを Supabase Auth（`grant_type=id_token`）でハッシュ化した nonce つきで交換します。保存されるのは、生成された Supabase セッションのみで、端末ローカルの Keychain に格納されます。メール/パスワードでのサインインは、意図的に提供していません（取り下げ）。 |
| **ゼロ知識の暗号化バックアップ** | 無料 | クライアント側の AES-256-GCM エンベロープです。ランダムなペイロード鍵は PBKDF2-HMAC-SHA256（21 万回）の鍵スロットでラップされます。Supabase `user_backups` にアップロードされるのは暗号文と秘密でないメタデータだけです（ユーザーごとに RLS）。サーバーは、ユーザーが保持する秘密なしには復号できません。 |
| **最小化されたバックアップのペイロード** | 無料 | 有効なブロックリストの ID、許可/ブロック済みのドメイン、リゾルバー設定、ローカルログの設定、Lava の外観などをバックアップします。`isPaid`、QA フラグ、診断データ、スナップショット、ブロックリストのバイトそのものは、明示的に除外します。 |
| **端末の秘密の鍵スロット** | 無料 | 端末だけの Keychain にある 32 バイトの端末秘密（`...ThisDeviceOnly`、iCloud には同期されません）で、同じ端末への復元をスムーズにします。 |
| **リカバリーコード + アシスト復元** | 無料 | 8 単語の CVCV コード（約 105 ビット）を、サーバーが保持する復元シェアと SHA256 で組み合わせて、アシスト復元スロットを解放します。二要素方式で、どちらか片方だけでは復号できません。 |
...
| **Live Activity のアクションボタン** | 無料 | N 分の一時停止（設定した長さ、デフォルト 5 分）、再開、再接続 — アプリのプロセス内で `LavaProtectionCommandService` を通って動く `LiveActivityIntent` です。認証つきの一時停止のバリエーションは端末のローカル認証が必要です。 |
...
初回起動の流れで、ローカル VPN の設定をインストールし、適切なデフォルトを設定します。
...
| **ネットワークアクティビティ（設定 → 詳細設定）** | 無料 | ネットワーク / ランタイム / ユーザーの遷移を、上限付きのローカル限定イベントストリームで記録し、App Group で共有します（`NetworkActivityLog`）。アクティビティの面から **設定 → 詳細設定** へ移されました（「Nerd Stats」のあと、`SettingsRoute.networkActivity`）。`.activityViewing` のロックの裏にあり、独自のプライバシーパネル（「この iPhone にとどまります」、7 日間保持）を持ちます。 |
| **不具合レポート** | 無料 | ユーザーが起動するウィザードで、匿名化したバンドルを `POST /v1/bug-reports` に送ります。v1 ではドメイン履歴は含みません。バンドルにはビルドの出どころ（`appVersion`／`appBuild`／`sourceRevision`）と、接続性の正直なカウンターも含まれるようになりました。シェイクして報告でも開けます（`RageShakeDetector`）。 |
| **サブスクリプション管理** | Plus | 契約中のユーザーには、アップグレード画面に「サブスクリプションを管理」（自動更新プラン、`AppStore.showManageSubscriptions` 経由）、「購入を復元」、そして権限の有効期限が表示されます。 |
...
3 つのバンドルが 1 つの App Group `group.com.lavasec` を共有し、そこにコンパイルされる `lavasec-ios: Shared/` のソースフォルダーが併存します。

- **LavaSecApp**（`com.lavasec.app`） — SwiftUI のアプリシェル。このビルドでは、ルートが 2 タブの `TabView`（**ガード** + **設定**）で、フィルターとアクティビティはガードタブ配下の詳細画面として開けます（ネットワークアクティビティは現在、設定 → 詳細設定の配下にあります）。
