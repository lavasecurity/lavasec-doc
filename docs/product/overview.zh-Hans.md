...
iOS 之所以把它标成「VPN」，是因为数据包隧道是 App 能在系统层面过滤 DNS 的唯一办法——但 Lava 做的是 **DNS／拦截列表过滤**，不是流量转发。我们想把范围说清楚：Lava 是本地的 DNS 域名过滤，**并不**保证每一个恶意域名或网址都会被拦下来。它看到的是域名，不是页面路径，所以它没法在一个本身可信的主机上拦掉某一个坏页面。另外，防护也不是引导一结束就自动开启的——App 里的 **Guard** 标签页才是判断防护当前是否生效的权威依据。
