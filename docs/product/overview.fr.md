...
- **Listes de blocage sélectionnées, uniquement par URL source** — Lava ne publie que les URL des listes en amont (plus des hachages indicatifs pour l'identité de cache et l'audit) ; l'appareil récupère chaque liste via TLS et la traite en local sous des plafonds de taille/de règles, et Lava ne réplique ni ne sert jamais les octets de listes de blocage tierces. Les listes communautaires ne sont pas épinglées par hachage — TLS + l'URL sélectionnée constituent la frontière d'intégrité — tandis que le niveau garde-fou de sécurité de Lava reste appliqué par hachage. La valeur par défaut livrée active **Block List Basic** (`AppConfiguration.lavaRecommendedDefaults`, défini dans `OnboardingDefaults.swift`) ; les sources copyleft comme HaGeZi, OISD, AdGuard et 1Hosts sont en option. Voir [le filtrage DNS et les listes de blocage](../architecture/dns-filtering-and-blocklists.md).
